...
func (p *appearancePanel) HandleKey(msg tea.KeyMsg) (tea.Cmd, bool) {
	...
	case "enter", " ":
		...
		// Apply live first so the switch is visible even if the disk write
		// fails; persisting second keeps the choice across restarts. InitTheme
		// only flips lipgloss's cached background flag (no terminal I/O), so
		// it is safe to call mid-program.
		kit.InitTheme(value)
